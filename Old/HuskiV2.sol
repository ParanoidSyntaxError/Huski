...
contract Huski is IERC20, IERC20Metadata, Lottery, Context, Ownable
{
    ...
    uint256 public constant reflectTax = 1;
    uint256 public constant lotteryTax = 3;
    ...
    constructor() VRFConsumerBase(_vrfCoordinator, _linkToken)
    {
        ...
        _swapReceiver = 0x000000000000000000000000000000000000dEaD;
        ...
        //_reflectExclusions[uniswapV2Pair] = true;
    ...
    */
    ...
    function _lockLiquidity() private
    {
        ...
        uint256 rStartingHskiAmount = _rOwned[_swapReceiver];

        //Swap BNB for HSKI
        _swapBNBforHSKI(bnbAmount);

        uint256 rHskiAmount = _rOwned[_swapReceiver] - rStartingHskiAmount;

        //Uniswap factory doesn't allow reciever to be token contract address
        _rOwned[_swapReceiver] -= rHskiAmount;
    ...
    */
    ...
    function _transferValues(uint256 tAmount) private view returns (uint256, uint256, uint256)
    {
        ...
        uint256 tTax = (tAmount / 100) * (reflectTax + burnTax + lotteryTax);
    ...
    function _taxValues(uint256 tAmount) private view returns(uint256, uint256, uint256, uint256, uint256, uint256)
    {
        ...
        uint256 tReflect = (tAmount / 100) * reflectTax;
        uint256 tBurn = (tAmount / 100) * burnTax;
        uint256 tLottery = (tAmount / 100) * lotteryTax;
    ...
    function _transfer(address sender, address recipient, uint256 tAmount) private 
    {
        ...
        uint256 transferedAmount = tAmount;

        if(_taxExclusions[sender])
        {
            _tOwned[sender] -= tAmount;
            _rOwned[recipient] += _reflectionFromToken(tAmount);
        }
        else
        {
            (uint256 rAmount, uint256 rTransferAmount,) = _transferValues(tAmount);
            ...
            transferedAmount = rTransferAmount;
        }
