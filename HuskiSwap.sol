...
contract HuskiSwap
{
    ...
    function _swapBNBforLINK(uint256 bnbAmount, uint256 minLink, address receiver) internal
    {
    ...
    function _swapBNBforHSKI(uint256 bnbAmount, uint256 minHski, address receiver) internal
    {
    ...
    function _swapTaxFreeBNBforHSKI(uint256 bnbAmount, uint256 minHski, address receiver) private
    {
        _hski.setTaxExclusion(uniswapV2Pair, true);

        _swapBNBforHSKI(bnbAmount, minHski, receiver);
    ...
    function _removeLiquidity(uint256 lpTokens, uint256 minHski, uint256 minBnb, address sender) private returns (uint256, uint256)
    {
    ...
    function buyHSKI(uint256 minHski) external payable returns (bool)
    {
        ...
        _swapTaxFreeBNBforHSKI(value, minHski, sender);
    ...
    function addLiquidity(uint256 hskiAmount, uint256 minHski, uint256 minBnb) external payable returns (uint256, uint256, uint256)
    {
        ...
        _hski.rawTransfer(sender, address(this), hskiAmount);
        ...
        //Return unused HSKI
        _hski.rawTransfer(address(this), sender, hskiAmount - hskiSent);
    ...
    function removeLiquidity(uint256 lpTokens, uint256 minHski, uint256 minBnb) external returns (uint256, uint256)
    {
        ...
        (uint256 hskiReceived, uint256 bnbReceived) = _removeLiquidity(lpTokens, minHski, minBnb, sender);
    ...
    //Lock HSKI/BNB forever
    function liquidityLock() public payable
    {

    }

    //Tax free buy back and burn
    function liquidityBurn() public payable
    {
        require(msg.value > 0);

        uint256 bnbValue = msg.value;

        uint256 startBalance = _hski.balanceOf(address(this));

        _swapTaxFreeBNBforHSKI(bnbValue, 0, address(this));

        _hski.transfer(_hski.BURN_WALLET(), _hski.balanceOf(address(this)) - startBalance);
    }

    //Lock contract BNB in the HSKI LP
    function liquidityBuyBack() public
    {
        ...
        //Swap BNB for HSKI
        _swapTaxFreeBNBforHSKI(bnbAmount, 0, _swapReceiver);
        ...
        _hski.rawTransfer(_swapReceiver, address(this), hskiAmount);
